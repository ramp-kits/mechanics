import numpy as np
from submissions.tf_simulate.quick_features import *
from submissions.tf_simulate.simulation_model import Simulation
...
class FeatureExtractor(object):
    ...
    def __init__(self):
        ...
        self.models = []
        for i in range(3):
            self.models.append(Simulation(6))
    ...
    def transform(self, X_df):
        ...
        for i in range(n):
            ...
            X[i][0] = model([_n_burn_in + _n_lookahead])[0].numpy()
            X[i][1] = X_model[i]
            for p in range(len(model.p.numpy()[0])):
                X[i][p + 2] = model.p.numpy()[0][p]
        return X
