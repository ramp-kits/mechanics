...
# #     s = 0
# #     for i in t:
# #         s = np.sum(a * tf.sin(w * i + p))
# #     return s

# def f_x(a, w, p, t):
#     return np.sum(a * tf.cos(
#         np.tensordot(t, w,
#                      axes=np.ones(shape=(tf.shape(t, out_type=tf.int32)[0], tf.shape(w, out_type=tf.int32)[0]))
#                      ) + p))


# def f_y(a, w, p, t):
#     return np.sum(a * tf.sin(
#         np.tensordot(t, w,
#                      axes=np.ones(shape=(tf.shape(t, out_type=tf.int32)[0], tf.shape(w, out_type=tf.int32)[0]))
#                      ) + p))
